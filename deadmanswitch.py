...
# Payload - It will encrypt whatever file we specified. Uncomment code to also delete original.
def ExecPayload(filePath, encryptPass):
...
def CheckKey(c, delayTime, filePath, encryptPass, targetTime):
    ...
    if not tweets:
        if (time.time() >= targetTime): ExecPayload(filePath, encryptPass)
        else:
            print("No results, trying again after delay")
            time.sleep(delayTime)
            CheckKey(c, delayTime, filePath, encryptPass, targetTime)
    else:
        print("Dead Man switch de-activated, reverted to safe mode")
        exit()
...
def GetTargets():
    ...
    try: datetime.datetime.strptime(startTime, '%Y-%m-%d')
        ...
        print("Error, not a date, try again (format: %Y-%m-%d)")
        GetTargets()
    c.Since = startTime
    ...
    targetTime = (time.time() + (int(input("How many minutes to run before executing?\n>"))*60))
    c.Hide_output = True
    c.Store_object = True
    CheckKey(c, delayTime, filePath, encryptPass, targetTime)
